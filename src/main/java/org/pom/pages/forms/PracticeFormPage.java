...
public class PracticeFormPage extends BasePage {
    ...
    public PracticeFormPage enterDate(String date) {
        ...
        String os = System.getProperty("os.name"); //выбор операционной системы
        System.out.println("My OS is " + os);

        if (os.startsWith("Mac")) {
            dateOfBirthInput.sendKeys(Keys.COMMAND, "a");//для мака
        } else {
            dateOfBirthInput.sendKeys(Keys.CONTROL, "a"); //для виндоус
        }
    ...
    public PracticeFormPage selectSubjects(String[] subjects) {
        ...
        for (int i = 0; i < subjects.length; i++) {
            if (subjects[i] != null) {
                typeWithJSExecutor(subjectsInput, subjects[i], 0, 300);//ввели что-то в поле
                subjectsInput.sendKeys(Keys.ENTER);//нажали на энтер
            }
        }
        return this;
    }
    ...
    @FindBy(id = "example-modal-siyes-titel-lg")
    WebElement modalTitle;

    public PracticeFormPage assertSubmit(String title) {//убеждаемся что готовая форма есть
